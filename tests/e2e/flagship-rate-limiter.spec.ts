...
test("grounds a rate-limiter contradiction in exact board evidence and records the revision", async ({
  ...
  await expect(page.getByText("Design a globally distributed API rate limiter.")).toBeVisible();
  await page.getByRole("button", { name: /load signature board/i }).click();
  ...
  await expect(probe).toContainText("globally consistent");
  await expect(probe).toContainText("regional stores");
  await expect(page.getByText(/Evidence focus: US Redis \+ EU Redis/i)).toBeVisible();
  ...
  await expect(page.getByText(/coordination path now connects both regional stores/i)).toBeVisible();
  ...
  await expect(page.getByText(/quota must remain globally consistent/i)).toBeVisible();
  await page.getByRole("button", { name: /candidate revision/i }).click();
  await expect(page.getByText(/global quota coordinator/i)).toBeVisible();
...
test("requires explicit transcript consent before creating an anonymous session", async ({ page }) => {
  ...
  const enterButton = page.getByRole("button", { name: /enter interview room/i });
