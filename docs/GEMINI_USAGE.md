...
- Functional today: a role-aware introduction, persistent interview conversation, typed text, board-idle analysis, and direct board analysis, all independent of Live. Voice presentation is selected by default while the microphone stays muted. Supported browsers provide opt-in speech recognition and automatic read-aloud when Live is unavailable; quiet Text mode uses bounded application-owned follow-ups for non-board interviews. These fallbacks are not Gemini audio or model-generated conversation. The deployed Cloud Run service is configured with `GEMINI_MODE=real` and `ENABLE_GEMINI_LIVE=false`.
